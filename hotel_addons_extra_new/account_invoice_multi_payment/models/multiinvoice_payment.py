...
class AdvancePayment(models.Model):
    ...
    reconcile_amount = fields.Monetary(string='Reconcile Amount', store=True)
    ...
    @api.depends('journal_related_id', 'partner_advance_balance_payment_id')
    @api.onchange('journal_related_id', 'partner_advance_balance_payment_id')
    def _get_refund_state(self):
        print('=========================111111111111========')
        for val in self:
            print('=========================222222222========')
            if val.refund_reconcile_amount:
                print('=========================333333333333========')
                if val.journal_related_id:
                    print('=========================444444444444========')
                    if val.journal_related_id.state == 'posted':
                        val.refund_state = 'refunded'
                    elif val.journal_related_id.state != 'posted':
                        val.refund_state = 'not_refunded'
            else:
                print('=========================55555555555========')
                val.refund_state = 'no_refund'
    ...
    def full_refund(self):
        ...
        self.journal_related_id.action_post()
...
class AccountPayment(models.Model):
    ...
    def get_advance_value(self):
        for line in self.payment_invoice_ids:
            dic_val = line.invoice_id.invoice_outstanding_credits_debits_widget
            res = json.loads(dic_val)
            for val in res['content']:
                for ad_val in self.advance_payment_ids:
                    if val['journal_name'] == ad_val.payment_name:
                        val['amount'] -= ad_val.reconcile_amount
            final = str(res)
            line.invoice_id.update({
                'invoice_outstanding_credits_debits_widget': res,
            })
            print('=================================================', final, type(final))

    @api.depends('advance_payment_ids.reconcile_amount', 'payment_invoice_ids')
    @api.onchange('advance_payment_ids.reconcile_amount', 'payment_invoice_ids')
    def get_total_advance_amount(self):
        for rec in self:
            ...
            # rec.amount = amount
            for advance in rec.advance_payment_ids:
                amount += advance.reconcile_amount
            rec.amount = amount
        ...
        if self.advance_payment_ids:
            for advanced in self.advance_payment_ids:
                ...
                advanced.partner_advance_balance_payment_id.write({
                    'advance_balance_amount': advanced.total_adv_balance_amount
                })
    ...
    def get_advance_balance_amount(self):
        total_amount = 0
        invoice_id = self.env['account.move'].search([('id', 'in', self.reconciled_invoice_ids.ids)])
    ...
    def _onchange_reconcile_amount(self):
        self.advance_balance_amount = self.amount
    #     payment_invoice_values = []
    #     extra_amount = 0
    ...
    def action_post(self):
        ...
        self.get_advance_balance_amount()
        # self.reconciled_invoice_ids._compute_payments_widget_to_reconcile_info()
        ...
        for move in self:
            ...
            for line in self.env['account.move.line'].search(domain):
                if line.currency_id == move.currency_id:
                    ...
                    # if line.payment_id.advance_balance_amount > 0:
                    amount = line.payment_id.advance_balance_amount
                else:
