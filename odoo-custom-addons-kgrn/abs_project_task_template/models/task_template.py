...
class Project(models.Model):
    ...
    use_task_template = fields.Boolean(string="Use Active Task Templates",
                                       help="Use Task Templates for creating Tasks of the Project")
        ...
        if vals.get('use_task_template'):
            ...
            if template_id:
                for template in template_id:
                    tasktemplate = {
                        'name': template.name, 'user_ids': [(6, 0, template.user_id.ids)],
                        'date_deadline': template.date_deadline, 
                        'description': template.description,
                        'project_id': variable.id}
