...
class HrPayrollyear(models.Model):
    ...
    # @api.constrains('day_and_month')
    # def _check_month(self):
    #     for record in self.day_and_month:
    #         if record.select_month:
    #             domain = [('name', '=', record.select_month)]
    #             name = self.search(domain)
    #             if len(name) > 1:
    #                 for i in range(len(name)):
    #                     if name[i].id != record.id:
    ...
    #     employee_public_leave = self.env['hr.holidays.public'].search(
    #         [('line_ids.month', '=', self.day_and_month.select_month)])
    #     for line in employee_public_leave.line_ids:
    #         line.name = self.day_and_month.holiday_public

    @api.onchange('day_and_month')
    def _public_holiday(self):
        for record in self.day_and_month:
            public_leave_count = 0.00
            employee_public_leave = record.env['hr.holidays.public.line'].sudo().search(
