...
class HrPayslip(models.Model):
    ...
    payslip_allowance_deduction = fields.Float(string='Allowance Deducted Amount')

    @api.onchange('line_ids', 'payslip_allowance_deduction')
    def contract_amount_subtraction(self):
        contract_valdiate = self.env['hr.contract'].sudo(). \
            search([('employee_id', '=', self.employee_id.id)])
        if self.employee_id:
            allowance_amount_diff = contract_valdiate.house_rent_allowance + \
                                    contract_valdiate.convenyance_allowance + \
                                    contract_valdiate.special_allowance + \
                                    contract_valdiate.travel_incentives + \
                                    contract_valdiate.health_insurance
            total_dedu_amount = 0
            sum = 0
            ...
            self.payslip_allowance_deduction = abs(total_dedu_amount)
